...
struct YelpFusionAPIResponse: Codable {
    var businesses: [Destination]
}

actor YelpFusionAPIService {
    // query businesses from the Yelp Fusion API, returning a list of businesses or throwing an error
    func businesses(searchingFor term: String, at location: CLLocationCoordinate2D) async throws -> [Destination] {
        ...
        let (data, httpresponse) = try await URLSession(configuration: .default).data(for: request)
        print(httpresponse)
